...
const [formData, setFormData] = useState({
  ...
  experiences: [
    {
      ...
      title: "",
      date: "",
      bullets: ["", ""], // empty bullets ready to fill
    },
  ],
  leadership: [
    {
 ...
 const deleteBulletFromExperience = (expIndex: number, bulletIndex: number) => {
   setFormData((prev) => {
     ...
     if (newExperiences[expIndex].bullets.length === 0) newExperiences[expIndex].bullets.push("");
     return { ...prev, experiences: newExperiences };
   });
 };
 ...
 const deleteBulletFromLeadership = (leadIndex: number, bulletIndex: number) => {
   setFormData((prev) => {
     ...
     if (newLeadership[leadIndex].bullets.length === 0) newLeadership[leadIndex].bullets.push("");
     return { ...prev, leadership: newLeadership };
   });
 };
 ...
 const deleteLeadership = (index: number) => {
   if (formData.leadership.length === 1) return;
   setFormData((prev) => {
     const newLeadership = [...prev.leadership];
       ...
       <Page size="A4" style={styles.page}>
         ...
         <View style={styles.section}>
           ...
           <View style={styles.rowBetween}>
             <Text style={{ fontWeight: "bold", fontSize: 12 }}>{pdfData.educationSchool.toUpperCase()}</Text>
 ...
 <View style={{ marginTop: 10, marginBottom: 10 }}>
  {/* High School Name and Location on the same row */}
  <View style={styles.rowBetween}>
    <Text style={{ fontWeight: "bold", fontSize: 12 }}>
      {pdfData.highSchoolName.toUpperCase()}
    </Text>
    <Text style={{ fontSize: 12 }}>
      {pdfData.highSchoolLocation}
    </Text>
  </View>
  

{/* Degree and Graduation Date on the same row */}
<View style={styles.rowBetween}>
  <Text style={{ fontSize: 12 }}>
    {pdfData.highSchoolDegree}
  </Text>
  <Text style={{ fontSize: 12 }}>
    {pdfData.highSchoolDate}
  </Text>
</View>
         ...
         <View style={styles.section}>
           ...
           {pdfData.experiences.map((exp, index) => (
             <View key={index}>
               <View style={styles.rowBetween}>
                 <Text style={{ fontWeight: "bold", fontSize: 12 }}>{exp.org.toUpperCase()}</Text>
               ...
               {exp.bullets.map((bullet, bulletIndex) => (
                 <Text
                   ...
                   style={{
                     fontSize: 11,
                     marginLeft: 10,
                     marginBottom: bulletIndex === exp.bullets.length - 1 ? 8 : 2,
                     lineHeight: 1.2,
                   }}
                 >
                   • {bullet}
         ...
         <View style={styles.section}>
           ...
           {pdfData.leadership.map((lead, index) => (
             <View key={index}>
               <View style={styles.rowBetween}>
                 <Text style={{ fontWeight: "bold", fontSize: 12 }}>{lead.org.toUpperCase()}</Text>
               ...
               {lead.bullets.map((bullet, bulletIndex) => (
                 <Text
                   ...
                   style={{
                     fontSize: 11,
                     marginLeft: 10,
                     marginBottom: bulletIndex === lead.bullets.length - 1 ? 8 : 2,
                     lineHeight: 1.2,
                   }}
                 >
                   • {bullet}
         ...
         <View style={styles.section}>
           ...
           <Text style={styles.entryDetails}>
             <Text style={{ fontWeight: "bold" }}>Technical: </Text>{pdfData.skills}
           </Text>
           <Text style={styles.entryDetails}>
             <Text style={{ fontWeight: "bold" }}>Languages: </Text>{pdfData.languages}
           </Text>
           <Text style={styles.entryDetails}>
             <Text style={{ fontWeight: "bold" }}>Interests: </Text>{pdfData.interests}
           </Text>
         </View>
       </Page>
     </Document>
 ...
 return (
   <div className="container">
     <div className="form">
       ...
       <h3>Education</h3>
       <input type="text" name="educationSchool" value={formData.educationSchool} onChange={handleChange} placeholder="University / College" />
       <input type="text" name="educationLocation" value={formData.educationLocation} onChange={handleChange} placeholder="City, Country" />
       ...
       <input type="text" name="highSchoolName" value={formData.highSchoolName} onChange={handleChange} placeholder="High School Name" />
         ...
         <div key={index} className="experience-block">
           ...
           {exp.bullets.map((bullet, bulletIndex) => (
             <div key={bulletIndex} style={{ display: "flex", gap: "10px", alignItems: "flex-start" }}>
               ...
               <button type="button" onClick={() => deleteBulletFromExperience(index, bulletIndex)} style={{
                 padding: "5px 10px",
           ...
           <div style={{ marginTop: "5px" }}>
             ...
             <button type="button" onClick={() => deleteExperience(index)} style={{
               marginLeft: "10px",
               padding: "5px 10px",
               backgroundColor: formData.experiences.length === 1 ? "#ccc" : "#ff4444",
               color: "white",
               border: "none",
               borderRadius: "4px",
               cursor: formData.experiences.length === 1 ? "not-allowed" : "pointer"
         ...
         <div key={index} className="leadership-block">
           ...
           {lead.bullets.map((bullet, bulletIndex) => (
             <div key={bulletIndex} style={{ display: "flex", gap: "10px", alignItems: "flex-start" }}>
               ...
               <button type="button" onClick={() => deleteBulletFromLeadership(index, bulletIndex)} style={{
                 padding: "5px 10px",
                 backgroundColor: "#ff4444",
                 color: "white",
                 border: "none",
           ...
           <div style={{ marginTop: "5px" }}>
             ...
             <button type="button" onClick={() => deleteLeadership(index)} style={{
               marginLeft: "10px",
               padding: "5px 10px",
               backgroundColor: formData.leadership.length === 1 ? "#ccc" : "#ff4444",
               color: "white",
               border: "none",
               borderRadius: "4px",
               cursor: formData.leadership.length === 1 ? "not-allowed" : "pointer"
